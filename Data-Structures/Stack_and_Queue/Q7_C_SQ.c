  ...
  while (c != 0) {
    ...
    switch (c) {
      case 1:
        ...
        break;
      case 2:
        if(balanced(str)) printf("balanced!\n");
...
int balanced(char *expression) {
  ...
  for(int index=0; index < strlen(expression); index++) {
		if(expression[index] == '(' || expression[index] == '[' || expression[index] == '{') push(stack, expression[index]);
		else if(expression[index] == ')') {
			if(stack->ll.size > 0 && peek(stack) == '(') pop(stack);
			else return False;
		}
		else if(expression[index] == ']') {
			if(stack->ll.size > 0 && peek(stack) == '[') pop(stack);
	...
	if(stack->ll.size != 0) return False;
	return True;
}
void removeAllItemsFromStack(Stack *s) {
  if (s == NULL)
