...
int isStackPairwiseConsecutive(Stack *s) {
  ListNode* pTraverse = s->ll.head;
	...
	int difference;
	while(pTraverse && pTraverse->next) {
		pNextTraverse = pTraverse->next->next;
		if(pTraverse->item < pTraverse->next->item) difference = pTraverse->next->item - pTraverse->item;
		else difference = pTraverse->item - pTraverse->next->item;
