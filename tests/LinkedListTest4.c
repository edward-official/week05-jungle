...
void testMoveEvens_keepsOddsFrontAndOrder(void) {
  int init[] = {1, 2, 3, 4, 5};
  for (int i = 0; i < 5; i++)
    pushBack(init[i]);
...
void testNoOp_whenTooShort(void) {
  moveEvenItemsToBack(&l);
...
int main(void) {
  UNITY_BEGIN();
  RUN_TEST(testMoveEvens_keepsOddsFrontAndOrder);
  RUN_TEST(testNoOp_whenTooShort);
  return UNITY_END();
}
