...
void createQueueFromLinkedList(LinkedList *ll, Queue *q) {
  if(!ll || !ll->head || !q) return;
	ListNode* pTraverse = ll->head;
	int index, value;
	while(pTraverse) {
		index = q->ll.size;
		value = pTraverse->item;
		insertNode(&(q->ll), index, value);
		pTraverse = pTraverse->next;
	}
}
void removeOddValues(Queue *q) {
	ListNode *pTraverse =  q->ll.head;
	ListNode *pNextTraverse;
	int index = 0;
	while(pTraverse) {
		pNextTraverse = pTraverse->next;
		if(pTraverse->item % 2 != 0) removeNode(&(q->ll), index);
		else index++;
		pTraverse = pNextTraverse;
	}
}
void enqueue(Queue *q, int item) {
