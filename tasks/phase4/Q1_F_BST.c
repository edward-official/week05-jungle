...
void levelOrderTraversal(BSTNode *root) {
  Queue *pCurrentLevel = malloc(sizeof(Queue)); pCurrentLevel->head = NULL; pCurrentLevel->tail = NULL;
  Queue *pNextLevel = malloc(sizeof(Queue)); pNextLevel->head = NULL; pNextLevel->tail = NULL;
	enqueue(&pCurrentLevel->head, &pCurrentLevel->tail, root);
	
	BSTNode *popped;
	while(pCurrentLevel->head) {
		while(pCurrentLevel->head) {
			popped = dequeue(&pCurrentLevel->head, &pCurrentLevel->tail);
			printf("%d ", popped->item);
			if(popped->left) enqueue(&pNextLevel->head, &pNextLevel->tail, popped->left);
			if(popped->right) enqueue(&pNextLevel->head, &pNextLevel->tail, popped->right);
		}
		while(pNextLevel->head) enqueue(&pCurrentLevel->head, &pCurrentLevel->tail, dequeue(&pNextLevel->head, &pNextLevel->tail));
	}
	printf("\n");
}
